    ...
    try {
      ...
      if (bans.size === 0) {
        return message.reply({
          embeds: [
            new EmbedBuilder()
              .setDescription(`${client.emotes.cross} | There are no banned users in this server.`)
              .setColor(client.color),
          ],
        });
      }
      ...
      collector.on("collect", async (i) => {
        ...
        if (i.customId === "yes") {
          ...
          const cancelEmbed = new EmbedBuilder()
            .setDescription(`${client.emotes.cross} | Unban all action canceled`)
            .setColor(client.color);
...
async function processBatch(bans, progressMsg, client, message) {
  ...
  for (const [userId, banInfo] of bans) {
    try {
      ...
      if (progressMsg) {
        await progressMsg.edit({ embeds: [progressEmbed] }).catch(async (error) => {
